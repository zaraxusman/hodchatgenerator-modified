...
import { SMSConfig, SMSMessage, smsTemplates, generateRandomSMS } from "@/types/sms";
import { defaultWallpaper } from "@/types/wallpaper";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Trash2, Plus, ArrowUp, ArrowDown, Shuffle, Sparkles } from "lucide-react";
...
export default function SMSBuilder({ config, onChange }: Props) {
  const [newMsg, setNewMsg] = useState("");
  const [newSender, setNewSender] = useState<"me" | "other">("other");

  const updateMessages = (messages: SMSMessage[]) => {
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
              newSender === "me" ? "bg-[#34c759]/30 text-foreground" : "bg-muted text-muted-foreground"
            }`}
          >
            Me
          </button>
          ...
          {config.messages.map((msg, i) => (
            <div
              ...
              className={`flex items-center gap-2 p-2 rounded-lg text-sm ${
                msg.sender === "me" ? "bg-[#34c759]/10" : "bg-[#3a3a3c]/30"
              }`}
            >
